...
<div class="modal fade change_header_image" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
...
<div class="modal fade emailunsent" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
  <div class="modal-dialog" role="document" style="width:40%">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">CRO ARD – B1 Notification Email</h4>
      </div>
      <div class="modal-body">
          <div class="row">
            <div class="col-md-1">
...
<div class="modal fade global_core_call_modal" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" data-backdrop="static" data-keyboard="false" style="margin-top: 5%;z-index: 999999;">
  <div class="modal-dialog modal-md" role="document" style="width:40%">
        <div class="modal-content">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
              <h4 class="modal-title">Global Core Call</h4>
            </div>
            <div class="modal-body" style="clear:both"> 
              <div class="col-md-12" style="max-height: 600px;overflow-y: scroll;scrollbar-color: #3db0e6 #f5f5f5;scrollbar-width: thin;"> 
            ...
            <div class="modal-footer" style="clear:both">  
                <input type="button" name="global_core_call" class="common_black_button global_core_call" value="Start Global Core Call" style="margin-top: 10px;float:right"> 
            </div>
        </div>
  </div>
</div>
...
<div class="page_title" style="z-index:999;">
  <h4 class="col-lg-5 padding_00 new_main_title"> CRO - ARD Manager </h4>
  <div class="row">
      <!-- Merged code snippet -->
      <div class="col-lg-12" style="display: flex; align-items: center;  border-bottom: 0px solid #c3c3c3;">
          ...
          <div class="col-lg-3" style="padding: 0;">
              ...
              <input type="button" name="global_core_call_btn" class="common_black_button global_core_call_btn" value="Global Core Call" style="margin-top: 10px;"> 
...
<ul class="nav nav-tabs" id="myTab" role="tablist" style="margin-top: 15px;">
  <li class="nav-item active">
    <a class="nav-link active" id="first-tab" data-toggle="tab" href="#first" role="tab" aria-controls="first" aria-selected="true">Full CRO Listing</a>
  </li>
  <li class="nav-item">
    <a href="<?php echo URL::to('user/croard_monthly'); ?>" class="nav-link">CRO Monthly listing</a>
  </li>
</ul>
                ...
                if($client->ard == "")
                {
		          ...
		          ?>
		            <tr class="edit_task <?php echo $disabled; ?>" style="<?php echo $style; ?>"  id="clientidtr_<?php echo $client->client_id; ?>">
		                ...
		                <td style="<?php echo $style; ?>" align="left"><spam class="clientid_sort_val"><?php echo $client->client_id; ?></spam><br>
                      <spam><img class="active_client_list_tm1" data-iden="<?php echo $client->client_id; ?>" src="<?php echo URL::to('public/assets/images/active_client_list.png'); ?>" style="width:24px; cursor:pointer;" title="Add to active client list" /></spam></td>
                    ...
                    <td class="future_submission_td" align="left" style="color:#000">
                      ...
                      if(($tasks_det))
                      {
                        echo '<div class="col-md-12 padding_00 linked_tasks_div" style="margin-top:15px"><h5>Linked Tasks</h5>';
                        $i = 1;
                        foreach($tasks_det as $task_det)
                        {
                          $task_details = DB::table('taskmanager')->where('practice_code', Session::get('user_practice_code'))->where('id',$task_det->task_id)->first();
                          echo '<p style="float: left;margin-top: 10px;font-weight: 600;width: 100%;">'.$i.'. Task : '.$task_details->taskid.' - '.$task_details->subject.'</p>';
                          $i++;
                        }
                        echo '</div>';
                      }
                      ...
                      ?>
                      <div class="col-md-12 padding_00" style="margin-top:15px">
                        <input type="checkbox" name="signature_file_check" id="signature_file_check<?php echo $client->client_id; ?>" class="signature_file_check" data-element="<?php echo $client->client_id; ?>" data-crono="<?php echo $client->cro; ?>" value="" <?php echo $signature_checked; ?>><label for="signature_file_check<?php echo $client->client_id; ?>" 
...
function update_gcc_finishtime(gccid = 0) {
  $.ajax({
      url:"<?php echo URL::to('user/update_gcc_finishtime'); ?>",
      type:"post",
      data:{gccid:gccid},
      success:function(result)
      {
...
function refresh_all_function(ival,gccid = 0)
{
	var ival = ival + 1;
  ...
  if(cro == "")
  {
    $.ajax({
        ...
        type:"post",
        data:{clientid:clientid,gccid:gccid},
        success:function(result)
        {
          $("#clientidtr_"+clientid).find(".cro_ard_td").html('');
          $("#clientidtr_"+clientid).find(".company_blue").html('').removeClass('company_blue');
          var ivali = ival + 1;
            if(ivali == countval) { update_gcc_finishtime(gccid); $("body").removeClass("loading_content"); $.colorbox.close(); }
          else { 
            setTimeout(function() {
              refresh_all_function(ival,gccid); 
            },500);
          }
        }
    });
    ...
    if(type == "Ltd" || type == "ltd" || type == "Limited" || type == "Limted" || type == "limited")
    {
        ...
        dataType:"json",
        type:"get",
        data:{clientid:clientid,cro:cro,gccid:gccid},
        success:function(result)
        {
...
$(window).click(function(e) {
  ...
  if($(e.target).hasClass('global_core_call_btn'))
  {
    $("body").addClass("loading");
    $.ajax({
      url:"<?php echo URL::to('user/show_global_corecall_details'); ?>",
      type:"post",
      success:function(result) {
        $("#gcc_tbody").html(result);
        $(".global_core_call_modal").modal("show");
        $("body").removeClass("loading");
      }
    });
      ...
      if (CKEDITOR.instances.editor_2)
      {
        ...
        if(comments == "")
        {
          ...
          if($(".2_bill_task").is(":checked"))
          {
            ...
            $("body").addClass("loading");
            $.ajax({
                url: "<?php echo URL::to('user/create_new_taskmanager_task_croard'); ?>",
                type: 'POST',
                data: formData,
                dataType:"json",
                success: function (result) {
                  var client_id = $("#client_search_task").val();
                  var linked_tasks = $("#clientidtr_"+client_id).find(".future_submission_td").find(".linked_tasks_div").html();
                  if(linked_tasks == ""){
                    var linked_tasks = $("#clientidtr_"+client_id).find(".future_submission_td").find(".linked_tasks_div").show();
                    $("#clientidtr_"+client_id).find(".future_submission_td").find(".linked_tasks_div").html('<h5>Linked Tasks</h5><p style="float: left;margin-top: 10px;font-weight: 600;width: 100%;">1. '+result['output']+'</p>');
                  }
                  else{
                    var linked_tasks_count = $("#clientidtr_"+client_id).find(".future_submission_td").find(".linked_tasks_div").find("p").length;
                    var next_count = linked_tasks_count + 1;
                    var linked_tasks = $("#clientidtr_"+client_id).find(".future_submission_td").find(".linked_tasks_div").show();
                    $("#clientidtr_"+client_id).find(".future_submission_td").find(".linked_tasks_div").append('<p style="float: left;margin-top: 10px;font-weight: 600;width: 100%;">'+next_count+'. '+result['output']+'</p>');
                  }
  ...
  if($(e.target).hasClass('yes_proceed'))
  {
    ...
    $(".global_core_call_modal").modal("hide");

    setTimeout(function() {
      $.ajax({
        url:"<?php echo URL::to('user/global_core_call_entry'); ?>",
        type:"post",
        success:function(result) {
          var gccid = result;
          setTimeout(function() {
