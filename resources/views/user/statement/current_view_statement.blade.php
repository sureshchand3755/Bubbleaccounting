              ...
              <div class="row">
                <div class="col-md-12" style="margin-bottom:10px; margin-left:2px;">
                  ...
                  <select name="select_months_in_stmt_funcs" class="select_months_in_stmt_funcs form-control" style="">
                    <?php
                    echo '<option value="">Select Month and Year</option>
                    <option value="'.date('Y-m-01', strtotime(date('Y-m-d'))).'">'.date('M-Y', strtotime(date('Y-m-d'))).'</option>';
                    ?>
                  </select>
                </div>
                <div class="col-md-12">
    ...
    <div class="row">
      <ul class="nav nav-tabs" id="myTab" role="tablist">
        <li class="nav-item active">
          <a class="nav-link active" id="trial-balance-tab" href="javascript:">Current View</a>
        </li>
        <li class="nav-item">
...
function next_set_clients_statement_loaded(page){
  var current_month = $(".select_month_values").val();
  $.ajax({
    url:"<?php echo URL::to('user/load_statement_clients_current_view'); ?>",
    type:"post",
    data:{current_month:current_month,page:page},
    dataType:"json",
    success:function(result)
    {
      ...
      if(result['pagecount'] == page){
        ...
        //scrolldiv(); 
        $(".stmt_funcs").prop("disabled",false); 
        $(".select_months_in_stmt_funcs").html(result['options']);
        $("body").removeClass("loading_content");
      }
      else{
...
$(window).click(function(e) {
  ...
  if($(e.target).hasClass('load_clients'))
  {
    ...
    $.ajax({
      url:"<?php echo URL::to('user/load_statement_clients_current_view'); ?>",
      type:"post",
      data:{current_month:current_month,page:1},
      dataType:"json",
      success:function(result)
      {
  ...
  if($(e.target).hasClass('export_values')){
    ...
    var current_month = $(".select_month_values").val();
    $.ajax({
      url:"<?php echo URL::to('user/export_statement_clients_current_view'); ?>",
      type:"post",
...
function nextStmtAllBuild(month,chunk){  
  $.ajax({
    ...
    {
      $.each(result, function( index, value ) {
        $(".client_tr_"+result[index]['client_id']).find(".build_statement_td_"+result[index]['month_year']).html(result[index]['link']);
      });
      setupLoop();
    }
  });
 ...
 function setupLoop()
 {
  ...
  // }
    ...
    $.each($("#statement_client_tbody tr"), function(){
      ...
      var opening_bal = $(".client_tr_"+client_id).find(".build_statement_td_"+month).find("a").data('balance');
