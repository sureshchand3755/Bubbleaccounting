...
<div class="modal fade statement_functions_modal" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" data-backdrop="static" data-keyboard="false" style="margin-top: 5%;z-index: 999999;">
  <div class="modal-dialog modal-lg" role="document" style="width:40%">
        <div class="modal-content">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
              <h4 class="modal-title">Statement Functions</h4>
            </div>
            <div class="modal-body" style="clear:both">  
              <div class="row">
                <div class="col-md-12" style="margin-bottom:10px; margin-left:2px;">
                  <spam class="build_statement_spam"></spam>
                  <input type="hidden" name="select_months_in_stmt_funcs" id="select_months_in_stmt_funcs" class="select_months_in_stmt_funcs" value="">
                </div>
                <div class="col-md-12">
  ...
  <div style="width: 98%; float: left; position: fixed;" id="headerfixed">
    ...
    <div class="row">
      <ul class="nav nav-tabs" id="myTab" role="tablist">
        <li class="nav-item">
          <a class="nav-link" id="trial-balance-tab" href="<?php echo URL::to('user/statement_list'); ?>">Current View</a>
        </li>
        <li class="nav-item active">
          <a class="nav-link active" id="journal-listing-tab" href="javascript:">Multi Period</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" id="journal-listing-tab" href="<?php echo URL::to('user/full_view_statement'); ?>">Full View</a>
        </li>
        <li class="nav-item">
      ...
      <div class="col-md-12" style="margin-top:20px">
          ...
          <select name="to_month_values" class="to_month_values form-control" style="width:10%;margin-left:10px;float:right">
            <?php
            ...
            for($i= 1; $i<=$age; $i++)
            {
              ...
              if($opening_month->statement_monthly_to_period != ""){
                if($opening_month->statement_monthly_to_period == $datevalll){
                ...
                if(date('m-Y') == $dateval) { $selected = 'selected="selected"'; } 
                else { $selected = ''; }
              }
          ...
          <spam style="float:right;margin-top: 8px; margin-left:8px;">Second Period Month: </spam>

          <select name="from_month_values" class="from_month_values form-control" style="width:10%;margin-left:10px;float:right">
            <?php
            $current_month = date('M-Y');
            $current_monthh = date('m-Y');
            $curr_str_month = date('Y-m-01');
            ...
            for($i= 1; $i<=$age; $i++)
            {
              ...
              $datevall = date('M-Y', strtotime('first day of next month', strtotime($opening_bal_month)));
              $datevalll = date('Y-m-01', strtotime('first day of next month', strtotime($opening_bal_month)));
              if($opening_month->statement_monthly_from_period != ""){
                if($opening_month->statement_monthly_from_period == $datevalll){
                  $selected = 'selected="selected"';
                }
                else{
                  $selected = '';
                ...
                if(date('m-Y', strtotime('-2 MONTH')) == $dateval) { $selected = 'selected="selected"'; } 
                else { $selected = ''; }
              }
            ...
            <input type="hidden" value="<?php echo $opening_month->statement_monthly_from_period; ?>" id="hf_prev_selected_fromdate" />
            <input type="hidden" value="<?php echo $opening_month->statement_monthly_to_period; ?>" id="hf_prev_selected_todate" />
          <spam style="float:right;margin-top: 8px;">First Period Month: </spam>
      </div>
    </div>
  </div>
...
<div class="modal_load"></div>
  ...
  <div class="modal_load_clients" style="text-align: center;"> <p style="font-size:18px;font-weight: 600;margin-top: 27%;">Please wait until all the Clients Invoices, Receipts and Closing Balance are Processed.</p> </div>
...
//  var from_period=$(".from_month_values");
//  var arrange_fromperiod = from_period.children().toArray().reverse();
//  from_period.append(arrange_fromperiod);
//  var to_period=$(".to_month_values");
//  var arrange_toperiod = to_period.children().toArray().reverse();
//  to_period.append(arrange_toperiod);
});
  ...
  $.ajax({
    ...
    success:function(result)
    {
      if(count == 0){
        ...
        $(".build_statement_spam").html(result[0]['options']);
        $(".select_months_in_stmt_funcs").val(result[0]['optionvalue']);
      }
      $.each(result, function(index,value) {
        $(".client_tr_"+result[index]['client_id']).find("td").detach();
        $(".client_tr_"+result[index]['client_id']).append(result[index]['tdval_divide']);
      });
...
function next_set_clients_statement_loaded(page){
  var from_month = $(".from_month_values").val();
    var to_month = $(".to_month_values").val();
  $.ajax({
    url:"<?php echo URL::to('user/load_statement_clients_monthly_view'); ?>",
    type:"post",
    data:{from_month:from_month,to_month:to_month,page:page},
    dataType:"json",
    success:function(result)
    {
      $("#statement_client_tbody").append(result['output']);
      ...
      if(result['pagecount'] == page){
        ...
        //scrolldiv(); 
        $(".stmt_funcs").prop("disabled",false); 
        $(".build_statement_spam").html(result['options']);
        $(".select_months_in_stmt_funcs").val(result['optionvalue']);
        $("body").removeClass("loading_content");
      }
      else{
        var nextpage = parseInt(page) + 1;
        var finishedcount = parseInt(page) * 100;
        $("#count_first").html(finishedcount);
        next_set_clients_statement_loaded(nextpage);
      }
    }
  })
}
$(window).change(function(e) {
  if($(e.target).hasClass('from_month_values'))
  {
    var value = $(e.target).val();    
    var selected_from_date = $("#hf_prev_selected_fromdate").val();
    var selected_to_date = $("#hf_prev_selected_todate").val();
    if(selected_from_date != null && selected_from_date != ''){
      var r= confirm("Current Statements will be Deleted if you Change the Months – Do you wish to proceed with the Month Change?");
      if(r){
        $.ajax({
          url:"<?php echo URL::to('user/save_statement_from_month_value'); ?>",
          type:"post",
          data:{value:value},
          success:function(result){
            $(".to_month_values").html(result);
            $(".to_month_values").val("");
          }
        });
        ...
        setTimeout(function(){
          $.ajax({
            url:"<?php echo URL::to('user/clear_prev_loaded_data'); ?>",
            type:"post",
            data:{from_month:selected_from_date,to_month:selected_to_date},
            success:function(result){
              $("#hf_prev_selected_fromdate").val('');
              $("#hf_prev_selected_todate").val('');
        ...
        $(".from_month_values").val(selected_from_date);
        $(".to_month_values").val(selected_to_date);
      }
    }
    else{
      $.ajax({
  ...
  if($(e.target).hasClass('to_month_values'))
  {
    var from = $(".from_month_values").val();
    if(from == ""){
      alert("Please select the first month and select the last month");
    }
    else{
      var to = $(e.target).val();
      var selected_from_date = $("#hf_prev_selected_fromdate").val();
      var selected_to_date = $("#hf_prev_selected_todate").val();
      if(selected_to_date != null && selected_to_date != ''){
        var r= confirm("Current Statements will be Deleted if you Change the Months – Do you wish to proceed with the Month Change?");
        if(r){
          $.ajax({
            url:"<?php echo URL::to('user/save_statement_to_month_value'); ?>",
            type:"post",
            data:{from:from,value:to},
            success:function(result){
              if(result == 1){
                $(".to_month_values").val("");
                alert("The From Month you have selected should be lesser than the To month.")
              }
              else{
                $("body").addClass("loading");
                setTimeout(function(){
...
$(window).click(function(e) {
  ...
  if($(e.target).hasClass('build_statement'))
  {
    ...
    $.ajax({
      url:"<?php echo URL::to('user/build_statement_monthly_view'); ?>",
      type:"post",
  ...
  if($(e.target).hasClass('load_clients'))
  {
    ...
    if(from_month != '' && to_month != ''){
      ...
      $.ajax({
        url:"<?php echo URL::to('user/load_statement_clients_monthly_view'); ?>",
        type:"post",
        data:{from_month:from_month,to_month:to_month,page:1},
        dataType:"json",
        success:function(result)
        {
          $("#load_table_clients").html(result['output']);
          $("#hf_prev_selected_fromdate").val(from_month);
          $("#hf_prev_selected_todate").val(to_month);


          if(result['pagecount'] == 1 || result['pagecount'] == 0){
            $(".load_values").prop("disabled",false);
            $(".stmt_funcs").prop("disabled",false);
            $(".statement_settings").show();
            //scrolldiv();
            ...
            $("#count_last").html(result['total_clients']); 
            next_set_clients_statement_loaded(2);
          }
        }
      })
    }
    else{
      alert("Please select the From and To month to load the datas");
    }
  }
  if($(e.target).hasClass('export_values')){
    ...
    if(from_month != '' && to_month != ''){
      ...
      $.ajax({
        url:"<?php echo URL::to('user/export_statement_clients_monthly_view'); ?>",
        type:"post",
      ...
      alert("Please select the From and To month to export the datas");
    }
  }
  ...
  if($(e.target).hasClass('invoice_td'))
  {
    ...
    $.ajax({
      ...
      type:"post",
      dataType:"json",
      data:{client:client,month:month,get_data_from:'month_period'},
      success:function(result)
      {
  ...
  {
    var select_month_val=$(".select_months_in_stmt_funcs").val();
    if(select_month_val != "")
    {
      ...
      alert("Please select the month and year to build the statements for all the clients");
    }
  }
  if($(e.target).hasClass('delete_all'))
  {
    var from_month = $(".from_month_values option:selected").text();
    var to_month = $(".to_month_values option:selected").text()
    var alert_text = from_month+" to "+to_month;
    var select_month_val=$(".select_months_in_stmt_funcs").val();
    if(select_month_val != "")
    {
      var r = confirm("You are about to Delete All the Statements for "+alert_text+", Do you wish to continue?");
  		if(r)
  		{      
        $("body").addClass("loading");
        $.ajax({
          url:"<?php echo URL::to('user/deleteall_statement_monthly'); ?>",
          type:"post",
          data:{month:select_month_val},
          success:function(result)
          {
            ...
            setTimeout(function() {
              ...
              //   $(this).parents(".client_tr").find(".build_statement_td").html('<a href="javascript:" name="build_statement" class="common_black_button build_statement" data-client="'+client_id+'" data-element="'+month_year+'" data-balance="'+opening_bal+'">Build</a>');
              // });
      ...
      alert("Please select the month and year to delete the statements for all the clients");
    }
  }
...
function nextStmtAllBuild(month,chunk){  
  ...
  $.ajax({
    url:"<?php echo URL::to('user/build_all_statement_monthly_view'); ?>",
    type:"post",
    ...
    {
      $.each(result, function( index, value ) {
        $(".client_tr_"+result[index]['client_id']).find(".build_statement_td_"+month).html(result[index]['link']);
      });
      setupLoop();
    }
  });
 ...
 function setupLoop()
 {
  ...
  // }
    ...
    $.each($("#statement_client_tbody tr"), function(){
      ...
      console.log(client_id+' - '+select_month);
      var opening_bal = $(".client_tr_"+client_id).find(".build_statement_td_"+select_month).find("a").data('balance');
    ...
    if(temp<=loop){
      ...
      nextStmtAllBuild(select_month, arrayOfArrays[temp]);
      temp++;
    }
    else{
